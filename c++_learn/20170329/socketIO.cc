...
namespace wd
{
ssize_t SocketIO::readn(char *buf, size_t count)//将数据从Buf之中读出来，然后写进sockfd_
{
    size_t nleft = count;  //剩余的字节数
    ssize_t nread; //用作返回值
    char *bufp = (char*)buf; //缓冲区的偏移量

    while(nleft > 0)//套接字缓存区的大小有限，每次最多只能读入比如4096个字节
    {
...
ssize_t SocketIO::writen(const char *buf, size_t count)//从sockfd_中取得数据往buf里面写
{
    size_t nleft = count;
    ssize_t nwrite;
    const char *bufp = buf;
